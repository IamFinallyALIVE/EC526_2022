...
int main(int argc, char **argv )
{
  ...
  // double R_cut = 2.0;  Used when resticting force to short range.
  //double  R_skin = 4.0;
  
  double dt = 0.1;
  int MaxIter = 2000;
  int skip =    10;
  FILE * pfile1 = fopen ("op.out","w"); 
...
#if 0 // check intilization  to debug (Should put this printing into separate function call
...
#if 1   //Print to file for Jupyter Graphics
      if(iter%skip == 0)
	{    
	  for (int j = 0; j < N; j++)
	    {
	      fprintf(pfile1,"%14.8f,%14.8f\t", pos[0+j*nd],pos[1+j*nd]);	   
	    }
	  fprintf(pfile1,"\n");
	}
#endif  
